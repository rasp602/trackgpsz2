...
class GpsController
{
	...
	public function Crud()
	{
		...
		if (isset($_REQUEST['idGps']) && $_REQUEST['idGps'] != '') {
	...
	public function Guardar()
	{
		...
		$vte->idGps = isset($_REQUEST['idGps']) ? trim($_REQUEST['idGps']) : '';
		...
		if ($vte->idGps != '') {
			if ($vte->idGps != $vte->imei) {
				...
				if ($consultaRepetido) {
					header('Location: ?c=gps&a=Crud1&idGps=' . urlencode($vte->idGps) . '&repetido=1');
					exit;
				}
			}
