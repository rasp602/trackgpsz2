...
// ------------------------------------------------------------
$hoja->setCellValue('A1', 'IMEI GPS');
$hoja->setCellValue('B1', 'SIM CARD GPS');
$hoja->setCellValue('C1', 'MODELO GPS');
...
while ($row = $result->fetch_assoc()) {
   ...
   // $estado = ($row['estadoPersona'] == 'A') ? "Activo" : "Inactivo";
    ...
    $hoja->setCellValue('A' . $filaExcel, $row['imeiGps']);
    $hoja->setCellValue('B' . $filaExcel, $row['simCardGps']);
    $hoja->setCellValue('C' . $filaExcel, $row['modelo']);
...
// ------------------------------------------------------------
$nombreArchivo = "reporte_gps_" . date("Y-m-d") . ".xlsx";
