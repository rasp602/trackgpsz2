...
{
    // Logo
    $this->Image('../../img/usuarios.png' , 30 ,8, 30 , 30,'PNG');
    ...
    $this->Cell(0, 8, 'REGISTRO DE PERSONAS', 0, 1, 'C');
    ...
    $this->SetWidths(array(10,30,50,40,70,40,40,40));

    $this->Row(array('#','CEDULA','PERSONA','FECHA CREADO','EMAIL','TELEFONO','ESTADO','ROL'));
...
$query3 = mysqli_query($con,
    "SELECT
     ...
     $where
     ORDER BY persona.numeroPersona ASC"
);
...
while ($fila = mysqli_fetch_assoc($query3)) {
    ...
    $estado = ($fila['estadoPersona'] == 'A') ? 'Activo' : 'Inactivo';
