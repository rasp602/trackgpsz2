...
$sql ="SELECT * FROM hotel $where ORDER by hotel.idHotel ASC";
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B1', 'HOTELES REGISTRADOS');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B6', 'CAPACIDAD');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['capacidadHotel']);
...
header('Content-Disposition: attachment;filename="hoteles registrados.xls"');
